<template>
    ...
    >
        ...
        <c-menu :location="menu.location">
            ...
            <c-item @click="uploadFile">上传文件</c-item>
            <c-item @click="downloadFile"
                    :is-disabled="!(menu.onFile && menu.chooseItem && menu.chooseItem.type === 0)">
                下载文件
            </c-item>
            ...
            <c-item @click="goto">在此处打开终端</c-item>
        </c-menu>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async uploadFile() {
                let cmd = new File('file', this.currentPath).upload()
                let res = await window.nodeFunc.uploadFile(this.$geneShellUrl(this.shell) + cmd, cbRes => {
                    this.$console(cbRes)
                })
                if (res.code === 200) {
            ...
            async downloadFile() {
                let cmd = new File(this.currentPath + this.menu.chooseItem.name).download()
                // let res = await this.$genRequest(this.shell, cmd)
                window.nodeFunc.downloadFile(this.$geneShellUrl(this.shell) + cmd, cdRes => {
                    // this.$console(cdRes)
                    switch (cdRes.code) {
                        case 200:
                            console.log('下载完成')
                        ...
                        case 201:
                            // console.log('下载进行中')
                            let {receive, total, progress, item} = cdRes
                            console.log(`收到数据: ${(receive / 1024 / 1024).toFixed(2)} / ${(total / 1024 / 1024).toFixed(2)}`)
                            console.log('下载进度' + (progress * 100).toFixed(0) + '%')
                            // if ((progress * 100).toFixed(0) > 50) {
                            //     item.pause()
                            // }
                            break
                        case 300:
            ...
            async createFile() {
                ...
                if (this.menu.createType) {
                    cmd = new File().createFolder
                } else {
                    cmd = new File().createFile
                }
                let res = await this.$genRequest(this.shell, cmd, [this.currentPath + '/' + this.menu.createName])
                await this.gotoPath(this.currentPath)
                this.isShowDialog = false
            },
            async rename($event) {
                let file = this.currentPath + '/' + this.menu.chooseItem.name
                let cmd = new File().rename
                let res = await this.$genRequest(this.shell, cmd, [file, $event.target.value])
            ...
            async deleteFile() {
                ...
                if (this.menu.chooseItem.type) {
                    cmd = new File().deleteFolder
                } else {
                    cmd = new File().deleteFile
                }
                let res = await this.$genRequest(this.shell, cmd, [this.currentPath + '/' + this.menu.chooseItem.name])
                await this.gotoPath(this.currentPath)
            },
